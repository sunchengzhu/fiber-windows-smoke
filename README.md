...
脚本逐笔等待 `Success`，并用醒目的 `BEFORE` / `AFTER - SUCCESS` 标题显示 channel 两端余额变化。完成后还会严格断言付款端减少量、收款端增加量、预期金额完全相等且直连手续费为零；任一条件不符都会让 CI 失败。GitHub Actions 运行时还会在 Job Summary 中生成一份 `B -> A -> Bottle` 资金流摘要。临时覆盖金额时可执行：
