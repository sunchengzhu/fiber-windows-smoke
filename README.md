...
6. 将支付前后余额、手续费、Payment Hash 和资金流写入日志及 GitHub Job Summary。

自动升级不会创建新 channel。需要数据库 migration 的版本会拒绝自动升级，不会直接修改现有数据。

## CI 如何判断成功

RPC 返回 `Success` 还不够。两笔支付都必须同时满足：
...
`ensure_channel` 可能锁定链上 CKB，日常不要勾选；`send_payment` 会执行真实的 `0.02 + 0.01 CKB` 支付。
